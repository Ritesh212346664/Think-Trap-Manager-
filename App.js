...
// — CONNECT CHANNEL
function PageConnect({ connected, setConnected }) {
        ...
        <div className="card">
          <div className="card-header">
            <div>
              ...
              <div className="card-sub">Link Think Trap to the automation engine</div>
            </div>
          ...
          {!connected ? (
            <>
              ...
              <div className="form-group">
                ...
                <input className="input" placeholder="UC..." value={channelId} onChange={e => setChannelId(e.target.value)} />
              </div>
              <div className="form-group">
                <div className="label">OAuth Client ID</div>
                <input className="input" placeholder="client_id.apps.googleusercontent.com" />
              </div>
              <div style={{ display: "flex", gap: 10 }}>
                <button className="btn btn-primary" onClick={() => setConnected(true)}>⚡ Connect Channel</button>
                <button className="btn btn-ghost" onClick={() => setConnected(true)}>Demo Mode</button>
              </div>
            </>
          ) : (
            <>
              <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 20 }}>
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: 16 }}>Think Trap</div>
                  <div style={{ color: "var(--sub)", fontSize: 13 }}>@thinktrap · 47,200 subscribers</div>
                  <div style={{ color: "var(--teal)", fontSize: 12, marginTop: 3 }}>✓ API Connected · Quota: 8,400 / 10,000</div>
                </div>
              </div>
              <div className="grid-3" style={{ marginBottom: 16 }}>
                {[["Total Videos", "127"], ["Total Views", "4.2M"], ["Watch Hours", "18,400"]].map(([l, v]) => (
                  <div key={l} style={{ background: "var(--surface)", borderRadius: 8, padding: "10px 14px", border: "1px solid var(--border)" }}>
                    ...
                    <div style={{ fontFamily: "var(--font-h)", fontSize: 22 }}>{v}</div>
                  </div>
                ))}
              </div>
...
function PageDashboard() {
  const stats = [
    { label: "Total Views", value: "4.2M", change: "+18%", up: true, color: "var(--accent)" },
    { label: "Subscribers", value: "47.2K", change: "+2,100", up: true, color: "var(--teal)" },
    { label: "Watch Time (hrs)", value: "18.4K", change: "+12%", up: true, color: "var(--gold)" },
    { label: "Avg CTR", value: "8.4%", change: "+1.2%", up: true, color: "var(--accent2)" },
  ];
  return (
    <div className="page fade-in">
      <div className="glow-line" />
      <div className="grid-4" style={{ marginBottom: 20 }}>
        {stats.map(s => (
          <div key={s.label} className="stat-card">
            ...
            <div className={`stat-change ${s.up ? "up" : "down"}`}>{s.up ? "▲" : "▼"} {s.change} this week</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid-2" style={{ marginBottom: 20 }}>
        <div className="card">
          <div className="card-header">
            <div><div className="card-title">📈 Subscriber Growth</div><div className="card-sub">Last 8 months</div></div>
          </div>
          <div style={{ height: 90 }}><LineSparkline data={MOCK_SUBS} color="var(--teal)" /></div>
        ...
        <div className="card">
          <div className="card-header">
            <div><div className="card-title">👁 Weekly Views</div><div className="card-sub">Last 7 days</div></div>
          </div>
          <BarChart data={MOCK_WEEKLY} color="var(--accent)" />
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "var(--sub)", marginTop: 4 }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead>
              <tr style={{ borderBottom: "1px solid var(--border)" }}>
                {["Title", "Views", "Likes", "Comments", "CTR", "Watch %", "Date"].map(h => (
            ...
            <tbody>
              {MOCK_VIDEOS.map(v => (
                <tr key={v.id} style={{ borderBottom: "1px solid var(--border)" }}>
                  ...
                  <td style={{ padding: "10px 12px", color: "var(--teal)", fontFamily: "var(--font-m)" }}>{(v.views/1000).toFixed(0)}K</td>
                  <td style={{ padding: "10px 12px", fontFamily: "var(--font-m)" }}>{(v.likes/1000).toFixed(1)}K</td>
                  <td style={{ padding: "10px 12px", fontFamily: "var(--font-m)" }}>{v.comments}</td>
                  <td style={{ padding: "10px 12px" }}>
                    <span className={`badge ${v.ctr > 8 ? "badge-teal" : "badge-gold"}`}>{v.ctr}%</span>
                  </td>
                  <td style={{ padding: "10px 12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      <div className="progress-bar" style={{ width: 60 }}>
                        <div className="progress-fill" style={{ width: `${v.watch}%` }} />
                      </div>
                      <span style={{ fontSize: 11, color: "var(--sub)" }}>{v.watch}%</span>
                    </div>
                  </td>
                  <td style={{ padding: "10px 12px", color: "var(--sub)", fontSize: 12 }}>{v.date}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
// — VOICEOVER
function PageVoice() {
  const [voice, setVoice] = useState("hi-m");
      ...
      <div className="grid-2">
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            ...
            <div className="form-group">
              <div className="label">Select Voice</div>
            ...
            <div className="form-group">
              ...
              <input type="range" min="0.5" max="2" step="0.1" defaultValue="1" style={{ width: "100%", accentColor: "var(--accent)" }} />
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <button className="btn btn-primary" onClick={simulate} disabled={playing}>
              {playing ? <><span className="spinner" /> Generating...</> : <>🎙 Generate Voiceover</>}
            </button>
            <button className="btn btn-ghost">📥 Download MP3</button>
          </div>
        </div>
        <div className="card" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div className="card-title">🔊 Preview</div>
          <div style={{ background: "var(--surface)", borderRadius: 12, padding: "20px", border: "1px solid var(--border)", textAlign: "center" }}>
            <div style={{ fontSize: 48, marginBottom: 12 }}>🎵</div>
            <div style={{ fontWeight: 700, marginBottom: 4 }}>{VOICE_OPTIONS.find(v => v.id === voice)?.label}</div>
            <div style={{ color: "var(--sub)", fontSize: 13, marginBottom: 16 }}>{playing ? "Generating audio..." : "Ready to generate"}</div>
            {playing && (
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
              <button className="btn btn-ghost btn-sm" disabled={!playing && true}>⏮</button>
              <button className="btn btn-primary btn-sm" onClick={simulate}>{playing ? "⏸" : "▶"}</button>
              <button className="btn btn-ghost btn-sm">⏭</button>
            </div>
          </div>
          <div className="card" style={{ padding: 14 }}>
            <div style={{ fontSize: 12, color: "var(--sub)", marginBottom: 8, fontWeight: 600 }}>ElevenLabs Voice API</div>
            {[["Model", "eleven_multilingual_v2"], ["Stability", "0.75"], ["Clarity", "0.85"], ["Est. Duration", "~45 seconds"]].map(([k, v]) => (
...
function PageUpload() {
  ...
  const steps = [
    { label: "Validate API credentials", status: step > 0 ? "done" : step === 0 ? "active" : "wait" },
    { label: "Upload video file (MP4)", status: step > 1 ? "done" : step === 1 ? "active" : "wait" },
    { label: "Set title, description & tags", status: step > 2 ? "done" : step === 2 ? "active" : "wait" },
    { label: "Upload thumbnail", status: step > 3 ? "done" : step === 3 ? "active" : "wait" },
    { label: "Set publish time", status: step > 4 ? "done" : step === 4 ? "active" : "wait" },
    { label: "Submit to YouTube API", status: step > 5 ? "done" : step === 5 ? "active" : "wait" },
    { label: "Video live ✓", status: step >= 6 ? "done" : "wait" },
  ];
  ...
  function simulate() {
    ...
    const t = setInterval(() => {
      i++;
      setStep(i);
      if (i >= 7) { clearInterval(t); setUploading(false); }
    }, 800);
  }
  ...
  return (
    <div className="page fade-in">
      ...
      <div className="grid-2">
        <div>
          <div className="card" style={{ marginBottom: 16 }}>
            <div className="card-title" style={{ marginBottom: 14 }}>📤 Upload Settings</div>
            {[["Title", "The Mirror Effect — Why You Hate People Like You"], ["Language", "Hinglish"], ["Category", "Education"], ["Privacy", "Public"], ["Publish Time", "Thu, Feb 20 · 7:00 PM IST"]].map(([l, v]) => (
            ...
            <div style={{ marginTop: 16, display: "flex", gap: 10 }}>
              <button className="btn btn-primary" onClick={simulate} disabled={uploading}>
                {uploading ? <><span className="spinner" /> Uploading...</> : <>⚡ Upload Now</>}
              </button>
        ...
        <div className="card">
          ...
          {steps.map((s, i) => (
            <div key={i} className="upload-step">
              <div className={`step-icon ${s.status === "done" ? "step-done" : s.status === "active" ? "step-active" : "step-wait"}`}>
                {s.status === "done" ? "✓" : s.status === "active" ? <span className="spinner" style={{ width: 14, height: 14 }} /> : i + 1}
              </div>
              <span style={{ fontSize: 13, color: s.status === "done" ? "var(--teal)" : s.status === "active" ? "var(--text)" : "var(--sub)" }}>{s.label}</span>
            </div>
          ))}
          {step >= 7 && (
            <div style={{ marginTop: 16, padding: "12px 16px", background: "rgba(0,212,170,0.1)", border: "1px solid rgba(0,212,170,0.3)", borderRadius: 10, fontSize: 13, color: "var(--teal)", fontWeight: 600 }}>
              🎉 Video uploaded successfully! Go live in 2-5 minutes.
            </div>
          )}
        </div>
      </div>
...
export default function App() {
  ...
  const pageMap = {
    connect: <PageConnect connected={connected} setConnected={setConnected} />,
        ...
        <div className="sidebar">
          ...
          <div style={{ padding: "16px 20px", marginTop: "auto", borderTop: "1px solid var(--border)" }}>
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap" }}>
              <span className="badge badge-teal" style={{ fontSize: 10, padding: "2px 7px" }}>Claude ✓</span>
              <span className={`badge ${connected ? "badge-teal" : "badge-red"}`} style={{ fontSize: 10, padding: "2px 7px" }}>YT {connected ? "✓" : "✗"}</span>
              <span className="badge badge-gold" style={{ fontSize: 10, padding: "2px 7px" }}>11Labs ✓</span>
            </div>
          </div>
        </div>
        ...
        <div className="main">
          <div className="topbar">
            ...
            <div className="topbar-actions">
              {connected && <span className="connected-badge"><span className="dot-pulse" /> Think Trap</span>}
